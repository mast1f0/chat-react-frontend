    ...
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-[40px] max-w-[500px] w-full h-[100%] max-h-55 z-50 p-6">
        <div className="flex-row">
          <button
            onClick={() => setTab(0)}
            className={` max-h-12 mr-3 py-2 rounded-[40px] min-w-[100px] text-sm transition-colors ${tab === 0
          ...
          <button
            onClick={() => setTab(1)}
            className={`px-4 py-2 min-w-[100px] rounded-[40px] max-h-12 text-sm transition-colors ${tab === 1
                ? "bg-[#403752] text-white"
                : "bg-[#f5f4f7] text-[#8C8098]"
              }`}
          >
